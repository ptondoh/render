/**
 * Page Mes Collectes
 * Affiche toutes les collectes de l'agent avec tableau paginé et recherche
 */
...
export default function MesCollectesPage() {
    ...
    function renderHeader() {
        ...
        header.className = 'bg-gradient-to-r from-blue-600 to-blue-800 rounded-lg shadow-lg p-6 mb-6';

        const title = document.createElement('h1');
        title.className = 'text-3xl font-bold text-white mb-2';
        title.textContent = 'Mes Collectes';

        const subtitle = document.createElement('p');
        subtitle.className = 'text-blue-100';
        subtitle.textContent = 'Historique complet de vos collectes de prix';
    ...
    function renderFilters() {
        ...
        marcheSelect.className = 'w-full rounded-lg border-gray-300 p-2 border focus:ring-2 focus:ring-blue-500';
        ...
        produitSelect.className = 'w-full rounded-lg border-gray-300 p-2 border focus:ring-2 focus:ring-blue-500';
        ...
        dateInput.type = 'date';
        dateInput.className = 'w-full rounded-lg border-gray-300 p-2 border focus:ring-2 focus:ring-blue-500';
        dateInput.value = searchFilters.date;
        ...
        resetBtn.addEventListener('click', () => {
            searchFilters = { marche: '', produit: '', date: '' };
            currentPage = 1;
            render();
        });
    ...
    function renderTable() {
        ...
        if (searchFilters.date) {
            filteredCollectes = filteredCollectes.filter(c => {
                const collecteDate = new Date(c.date).toISOString().split('T')[0];
                return collecteDate === searchFilters.date;
            });
        }

        // Grouper les collectes par marché, produit et date
        const groupedCollectes = {};
        filteredCollectes.forEach(c => {
            const key = `${c.marche_id}_${c.produit_id}_${new Date(c.date).toISOString().split('T')[0]}`;
        ...
        headerTitle.textContent = `Total: ${totalItems} collecte(s)`;
        ...
        if (paginatedItems.length === 0) {
            ...
            tr.innerHTML = '<td colspan="7" class="px-6 py-8 text-center text-gray-500">Aucune collecte trouvée</td>';
    ...
    function render() {
        ...
        if (isLoading) {
            ...
            loading.innerHTML = '<div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div><p class="mt-4 text-gray-600">Chargement...</p>';
